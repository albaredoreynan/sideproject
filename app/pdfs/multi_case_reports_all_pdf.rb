class MultiCaseReportsAllPdf < Prawn::Document
	include ActionView::Helpers::NumberHelper
	def initialize(case_listings, file_entries, start_date, end_date)
		...
		move_down(12)
		move_down(5)
		@case_listings.each do |clisting|
			case_entry_reports2(@start_date, @end_date, clisting.file_matter_id, clisting.file_matter_case)
		end
		#case_entry_reports2
	end

	def case_entry_reports2(start_date, end_date, file_matter_id, case_number)
		row1 = []
		row1 << [start_date, end_date, file_matter_id, case_number]
		  ...
		  row1 << [{:content => "", :colspan => 6 }]
			...
			@grand_total_payment = number_to_currency( @totals_all.inject(:+).to_f, :unit => @units)
				row1 << [
						...
						{:content => "#{@grand_total_payment.to_f}", :align => :right, :font_style => :bold}
						] 
		end
