class PagesController < ApplicationController
	def update_songs
		@file_matters = FileMatter.find(params[:case_entry_file_matter_id])
		#@file_matters = FileMatter.find(:all, :conditions => { :file_matter_id => params[:case_entry_file_matter_id] } )
		#@lawyers =  Lawyer.find(@file_matters.lawyer_id).map{|b|[b.first_name, b.id]}
		@my_clients =  Client.find(:all, :conditions => { :id => @file_matters.client_id } ).map{|b| [b.name, b.id]}
		# @my_lawyers =  Lawyer.find(:all, :conditions => { :id => @file_matters.lawyer_id } ).map{|b| [b.full_name, b.id]}

		
		@file_matters2 = FileMatter.find(:all, :conditions => { :file_code => @file_matters.file_code } )
		@lawyers = Array.new
		@file_matters2.each do |fm|
			@lawyers << fm.lawyer_id
		end
		@my_lawyers =  Lawyer.find(:all, :conditions => ["id IN (?)", @lawyers] ).map{|b| [b.full_name, b.id]}
		
	end

	def update_songs2
		@file_matters2 = FileMatter.find(:all, :conditions => { :case_number => params[:file_matter_case] } )
		@file_matters = FileMatter.find(:all, :conditions => { :case_number => params[:file_matter_case] } ).map{|b| [b.file_code, b.id]}
		@my_clients =  Client.find(:all, :conditions => { :id => @file_matters2 } ).map{|b| [b.name, b.id]}
		@my_lawyers =  Lawyer.find(:all, :conditions => { :id => @file_matters2 } ).map{|b| [b.full_name, b.id]}
	end
end